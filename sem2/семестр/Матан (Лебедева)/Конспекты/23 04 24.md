...
$r\ge k+1\Rightarrow\phi\in C^2(V_{(x_{i_k},x_{i_{k+1} })})\Rightarrow\phi''_{st},\phi''_{ts}\in C\Rightarrow$ по [теореме](23 04 24) получаем, что $\phi''_{st}=\phi''_{ts} ~~ \forall(s,t)\in V_{(x_{i_k},x_{i_{k+1} })}$.  
...
$f(x+h)=F(1)$ и $F^{(k)}(0)=d^kf(x,h)$ по [Леммe 0](23 04 24)  $\scriptsize\blacksquare$  
