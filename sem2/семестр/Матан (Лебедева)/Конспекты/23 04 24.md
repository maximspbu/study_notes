...
$r\ge k+1\Rightarrow\phi\in C^2(V_{(x_{i_k},x_{i_{k+1} })})\Rightarrow\phi''_{st},\phi''_{ts}\in C\Rightarrow$ по [теореме]({{ site.baseurl }}/sem2/семестр/Матан (Лебедева)/Конспекты/23 04 24/23 04 24) получаем, что $\phi''_{st}=\phi''_{ts} ~~ \forall(s,t)\in V_{(x_{i_k},x_{i_{k+1} })}$.  
...
Разложим $F(t)$ в точке $t=0$ по формуле [Тейлора с остатком в форме Лагранжа]({{ site.baseurl }}//www.notion.so/60-10d641f35a50438587937e2d6a5a6da5?pvs=21) и подставим точку $t=1$.  
$F(1)=\displaystyle\sum_{k=0}^d\dfrac{F^{(k)}(0)}{k!}(1-0)^k+\dfrac{F^{(r+1)}(\theta)}{(r+1)!}(1-0)^{(r+1)}$  
$f(x+h)=F(1)$ и $F^{(k)}(0)=d^kf(x,h)$ по [Леммe 0]({{ site.baseurl }}/sem2/семестр/Матан (Лебедева)/Конспекты/23 04 24/23 04 24)  $\scriptsize\blacksquare$  
