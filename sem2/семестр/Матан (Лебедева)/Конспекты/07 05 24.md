...
### Доказательство:  
$F:[0,1]\to\mathbb{R}^m,~F(t):=f(a+t(b-a)),~t\in[0,1].~F$ непрерывна и дифференцируема на $[0,1]$. По [теореме Лагранжа для вектор-функций](07 05 24):  
...
По [теореме Лагранжа для вектор-функций](07 05 24) для $F(t),~t\in[0,1]$  
...
Тогда по [теореме об обратимости оператора близкого к обратимому](30 04 24) получаем, что $f'(x)$ обратим $( * )$. Пусть $\big(f'(x)\big)^{-1}=C$.  
...
$T:\Omega(l_2^d)\to \Omega(l_2^d);\quad M\mapsto M^{-1}~-~$непрерывна по [следствию](07 05 24).  
