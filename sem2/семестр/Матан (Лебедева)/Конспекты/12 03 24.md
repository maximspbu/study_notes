...
### Доказательство: применяем [теорему](05 03 24) к $g(-x)$.  
...
### Доказательство:  
Если $g\searrow$, то применяем [теорему](05 03 24) к $g(x)-g(b)$.  
Если $g\nearrow$, то [следствие $1$.](12 03 24)  
...
$n=0$:  
Из формулы [Ньютона-Лейбница](05 03 24):  
$f(x)=f(x_0)+\displaystyle\int_{x_0}^x f'(t)dt; ~~ f\in C^1(a;b)\Rightarrow f'\in C(a;b)$.  
Индукционный переход (снова применим [ф-лу Ньютона-Лейбница](05 03 24)).  
...
Применим [1-ую теорему о среднем для интегралов](05 03 24).  
$\dfrac{1}{n!}\displaystyle\int_{x_0}^x f^{(n+1)}(t)(x-t)^n dt = \dfrac{1}{n!}f^{(n+1)}(c)\int_{x_0}^x (x-t)^n dt=$  
$=\dfrac{1}{n!}f^{(n+1)}(c)\Big(-\dfrac{(x-t)^{n+1} }{n+1}\Big)\Big|_{x_0}^x$ (из [формулы](05 03 24)) $=$  
...
### Доказательство:  
Возьмём $с\in[a;b]$. Для $f,g$ и $[a;c]$ применим [теорему об интегрировании по частям для интеграла Римана](05 03 24).  
...
[По теореме о замене переменной в интеграле Римана](05 03 24),  
