...
$n=0$:  
Из формулы [Ньютона-Лейбница](Конспекты/05 03 24):  
...
Применим [1-ую теорему о среднем для интегралов](Конспекты/05 03 24).  
$\dfrac{1}{n!}\displaystyle\int_{x_0}^x f^{(n+1)}(t)(x-t)^n dt = \dfrac{1}{n!}f^{(n+1)}(c)\int_{x_0}^x (x-t)^n dt=\\$  
$=\dfrac{1}{n!}f^{(n+1)}(c)\Big(-\dfrac{(x-t)^{n+1} }{n+1}\Big)\Big|_{x_0}^x$ (из [формулы](Конспекты/05 03 24)) $=$  
