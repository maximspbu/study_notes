...
$r\ge k+1\Rightarrow\phi\in C^2(V_{(x_{i_k},x_{i_{k+1} })})\Rightarrow\phi''_{st},\phi''_{ts}\in C\Rightarrow$ по [теореме]({{ site.baseurl }}/sem2/семестр/Матан (Лебедева)/Экзамен/Конспекты/23 04 24) получаем, что $\phi''_{st}=\phi''_{ts} ~~ \forall(s,t)\in V_{(x_{i_k},x_{i_{k+1} })}$.  
